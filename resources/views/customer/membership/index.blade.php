    ...
    @elseif($subs && $subs->status === 'pending')
      <div class="card bg-warning text-dark p-3">
        ...
        @if($admin && $admin->phone)
            <a href="https://wa.me/{{ $admin->phone }}?text=Halo%20Admin,%20saya%20{{ auth()->user()->name }}%20sudah%20ajukan%20membership%20Premium%20dan%20status%20saya%20pending.%20Mohon%20konfirmasi."
            class="btn btn-success">
            Hubungi Admin via WhatsApp
            </a>
            <p class="mt-2">Nomor Admin ({{ $admin->name }}): <strong>{{ $admin->phone }}</strong></p>
        @else
            <p class="text-muted">Nomor admin belum tersedia.</p>
        @endif
        </div>
